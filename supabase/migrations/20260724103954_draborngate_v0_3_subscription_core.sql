-- DraBornGate v0.3.0
-- Paketler, bir defalık Profesyonel deneme, kullanım limitleri ve ödeme/fatura temeli.
...
create index if not exists dkd_gate_site_subscriptions_status_idx on draborngate.dkd_gate_site_subscriptions(status,plan_code);
create index if not exists dkd_gate_payment_requests_status_idx on draborngate.dkd_gate_subscription_payment_requests(status,created_at desc);
create index if not exists dkd_gate_invoices_site_idx on draborngate.dkd_gate_subscription_invoices(site_id,issued_at desc);
create index if not exists dkd_gate_report_exports_site_idx on draborngate.dkd_gate_report_exports(site_id,created_at desc);

drop trigger if exists dkd_gate_subscription_plans_updated_at on draborngate.dkd_gate_subscription_plans;
create trigger dkd_gate_subscription_plans_updated_at before update on draborngate.dkd_gate_subscription_plans
...
create trigger dkd_gate_site_subscriptions_updated_at before update on draborngate.dkd_gate_site_subscriptions
for each row execute function draborngate.dkd_gate_set_updated_at();
...
create trigger dkd_gate_subscription_payment_requests_updated_at before update on draborngate.dkd_gate_subscription_payment_requests
for each row execute function draborngate.dkd_gate_set_updated_at();
...
insert into draborngate.dkd_gate_subscription_plans(
  ...
  allow_export,advanced_finance,priority_support,custom_branding,trial_days,is_public,is_active,sort_order
) values
...
on conflict(code) do update set
  ...
  yearly_price=excluded.yearly_price,currency=excluded.currency,site_limit=excluded.site_limit,
  gate_limit=excluded.gate_limit,staff_limit=excluded.staff_limit,resident_limit=excluded.resident_limit,
  ...
  report_days_limit=excluded.report_days_limit,allow_export=excluded.allow_export,
  advanced_finance=excluded.advanced_finance,priority_support=excluded.priority_support,
  custom_branding=excluded.custom_branding,trial_days=excluded.trial_days,
...
as $$
  select coalesce((
    select s.plan_code from draborngate.dkd_gate_site_subscriptions s
    where s.site_id=p_site_id and (
      ...
      or s.status='free'
    ) order by case s.status when 'active' then 0 when 'trialing' then 1 else 2 end limit 1
  ),'starter');
$$;
...
set search_path=draborngate,public,auth
as $$
  select p.* from draborngate.dkd_gate_subscription_plans p
...
set search_path=draborngate,public,auth
as $$
declare v_trial_days integer; v_claimed boolean;
begin
  if new.is_demo then
    ...
    ) values(new.id,'professional','trialing','monthly',now(),now()+interval '365 days',now(),now()+interval '365 days','demo','v0.3 örnek Profesyonel paket')
    on conflict(site_id) do nothing;
    return new;
  end if;
  select exists(select 1 from draborngate.dkd_gate_trial_claims where user_id=new.owner_user_id) into v_claimed;
  select trial_days into v_trial_days from draborngate.dkd_gate_subscription_plans where code='professional' and is_active;
  if not v_claimed and coalesce(v_trial_days,0)>0 then
    insert into draborngate.dkd_gate_site_subscriptions(
      site_id,plan_code,status,billing_cycle,current_period_start,current_period_end,
      trial_started_at,trial_ends_at,source,notes
    ) values(new.id,'professional','trialing','monthly',now(),now()+(v_trial_days||' days')::interval,now(),now()+(v_trial_days||' days')::interval,'trial','Otomatik Profesyonel deneme paketi')
    on conflict(site_id) do nothing;
    insert into draborngate.dkd_gate_trial_claims(user_id,site_id,plan_code)
    values(new.owner_user_id,new.id,'professional') on conflict(user_id) do nothing;
  else
    insert into draborngate.dkd_gate_site_subscriptions(site_id,plan_code,status,billing_cycle,current_period_start,source,notes)
    values(new.id,'starter','free','monthly',now(),'system','Ücretsiz Başlangıç paketi') on conflict(site_id) do nothing;
  end if;
  return new;
end;
$$;
...
create trigger dkd_gate_sites_assign_subscription after insert on draborngate.dkd_gate_sites
...
begin
  ...
  join draborngate.dkd_gate_subscription_plans p on p.code=draborngate.dkd_gate_effective_plan_code(s.id)
  where s.owner_user_id=new.owner_user_id and s.is_active and not s.is_demo;
  select count(*) into v_count from draborngate.dkd_gate_sites s
  where s.owner_user_id=new.owner_user_id and s.is_active and not s.is_demo;
...
begin
  ...
  select count(*) into v_count from draborngate.dkd_gate_site_gates where site_id=new.site_id and is_active and not is_demo;
  if coalesce(v_limit,1)>0 and v_count>=coalesce(v_limit,1) then raise exception 'Mevcut paketiniz en fazla % kapıya izin veriyor',coalesce(v_limit,1); end if;
  return new;
end;
$$;
...
set search_path=draborngate,public,auth
as $$
declare v_staff_limit integer; v_resident_limit integer; v_count integer;
begin
  if new.is_demo or draborngate.dkd_gate_is_admin_user(auth.uid()) then return new; end if;
  select staff_limit,resident_limit into v_staff_limit,v_resident_limit from draborngate.dkd_gate_effective_plan(new.site_id);
  if new.role in ('owner','manager','security') then
    select count(*) into v_count from draborngate.dkd_gate_site_memberships where site_id=new.site_id and is_active and not is_demo and role in ('owner','manager','security');
    if coalesce(v_staff_limit,3)>0 and v_count>=coalesce(v_staff_limit,3) then raise exception 'Mevcut paketiniz en fazla % yönetim ve güvenlik kullanıcısına izin veriyor',coalesce(v_staff_limit,3); end if;
  elsif new.role='resident' then
    select count(*) into v_count from draborngate.dkd_gate_site_memberships where site_id=new.site_id and is_active and not is_demo and role='resident';
    if coalesce(v_resident_limit,30)>0 and v_count>=coalesce(v_resident_limit,30) then raise exception 'Mevcut paketiniz en fazla % site sakinine izin veriyor',coalesce(v_resident_limit,30); end if;
  end if;
  return new;
end;
$$;
...
begin
  if new.is_demo or draborngate.dkd_gate_is_admin_user(auth.uid()) then return new; end if;
  select monthly_courier_pass_limit into v_limit from draborngate.dkd_gate_effective_plan(new.site_id);
  if coalesce(v_limit,100)=0 then return new; end if;
  select count(*) into v_count from draborngate.dkd_gate_courier_passes
  where site_id=new.site_id and not is_demo and created_at>=date_trunc('month',now()) and created_at<date_trunc('month',now())+interval '1 month';
  if v_count>=coalesce(v_limit,100) then raise exception 'Bu site aylık % kurye geçişi paket limitine ulaştı',coalesce(v_limit,100); end if;
  return new;
end;
$$;
...
begin
  if new.is_demo or draborngate.dkd_gate_is_admin_user(auth.uid()) then return new; end if;
  select monthly_visitor_pass_limit into v_limit from draborngate.dkd_gate_effective_plan(new.site_id);
  if coalesce(v_limit,50)=0 then return new; end if;
  select count(*) into v_count from draborngate.dkd_gate_visitor_passes
  where site_id=new.site_id and not is_demo and created_at>=date_trunc('month',now()) and created_at<date_trunc('month',now())+interval '1 month';
  if v_count>=coalesce(v_limit,50) then raise exception 'Bu site aylık % ziyaretçi geçişi paket limitine ulaştı',coalesce(v_limit,50); end if;
  return new;
end;
$$;

drop trigger if exists dkd_gate_sites_plan_limit on draborngate.dkd_gate_sites;
create trigger dkd_gate_sites_plan_limit before insert on draborngate.dkd_gate_sites for each row execute function draborngate.dkd_gate_enforce_site_limit();
drop trigger if exists dkd_gate_site_gates_plan_limit on draborngate.dkd_gate_site_gates;
create trigger dkd_gate_site_gates_plan_limit before insert on draborngate.dkd_gate_site_gates for each row execute function draborngate.dkd_gate_enforce_gate_limit();
drop trigger if exists dkd_gate_site_memberships_plan_limit on draborngate.dkd_gate_site_memberships;
create trigger dkd_gate_site_memberships_plan_limit before insert on draborngate.dkd_gate_site_memberships for each row execute function draborngate.dkd_gate_enforce_member_limit();
drop trigger if exists dkd_gate_courier_passes_plan_limit on draborngate.dkd_gate_courier_passes;
create trigger dkd_gate_courier_passes_plan_limit before insert on draborngate.dkd_gate_courier_passes for each row execute function draborngate.dkd_gate_enforce_courier_pass_limit();
drop trigger if exists dkd_gate_visitor_passes_plan_limit on draborngate.dkd_gate_visitor_passes;
create trigger dkd_gate_visitor_passes_plan_limit before insert on draborngate.dkd_gate_visitor_passes for each row execute function draborngate.dkd_gate_enforce_visitor_pass_limit();

revoke all on function draborngate.dkd_gate_effective_plan_code(uuid) from public;
revoke all on function draborngate.dkd_gate_effective_plan(uuid) from public;
