...
create sequence if not exists draborngate.dkd_gate_invoice_number_seq start 1;
...
begin
  ...
  select jsonb_build_object(
    ...
    'plan',to_jsonb(p),
    'pending_request',(
      select to_jsonb(r) from draborngate.dkd_gate_subscription_payment_requests r
      where r.site_id=p_site_id and r.status='pending' order by r.created_at desc limit 1
    ),
    'latest_invoice',(
      select to_jsonb(i) from draborngate.dkd_gate_subscription_invoices i
      where i.site_id=p_site_id order by i.issued_at desc limit 1
    ),
    ...
    'invoices',(
      select coalesce(jsonb_agg(to_jsonb(i) order by i.issued_at desc),'[]'::jsonb)
      from draborngate.dkd_gate_subscription_invoices i where i.site_id=p_site_id
    )
  ) into v_result
