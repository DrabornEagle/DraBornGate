...
begin
  ...
  v_summary := v_summary || (
    select jsonb_build_object(
      'average_approval_minutes',round(coalesce(avg(extract(epoch from (e.created_at-p.created_at))/60),0),1)
    )
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'date',d.day,
    'courier',coalesce(c.total,0),
    'completed',coalesce(c.completed,0),
    'rejected',coalesce(c.rejected,0),
    'visitor',coalesce(v.total,0)
  ) order by d.day),'[]'::jsonb) into v_daily
  from generate_series(v_from,v_to,interval '1 day') d(day)
  left join (
    select created_at::date day,count(*) total,
  ...
  select coalesce(jsonb_agg(jsonb_build_object('hour',h.hour,'total',coalesce(x.total,0)) order by h.hour),'[]'::jsonb)
  into v_hourly
  from generate_series(0,23) h(hour)
  left join (
    select extract(hour from created_at)::int hour,count(*) total
    from draborngate.dkd_gate_courier_passes
    where site_id=p_site_id and created_at>=v_from::timestamptz and created_at<(v_to+1)::timestamptz
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'completed',completed,
    'rejected',rejected,
    'completion_rate',round(coalesce(100.0*completed/nullif(total,0),0),1),
    'average_minutes',average_minutes
  ) order by total desc,gate),'[]'::jsonb) into v_gates
  from (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'platform',platform,'total',total,'completed',completed,'rejected',rejected
  ) order by total desc,platform),'[]'::jsonb) into v_platforms
  from (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'courier_user_id',courier_user_id,'courier_name',courier_name,'platform',platform,
    'total',total,'completed',completed,'rejected',rejected,'average_minutes',average_minutes
  ) order by total desc,courier_name),'[]'::jsonb) into v_couriers
  from (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'user_id',actor_user_id,'name',full_name,'approved',approved,'rejected',rejected,
    'completed',completed,'total_actions',total_actions,'average_decision_minutes',average_decision_minutes
  ) order by total_actions desc,full_name),'[]'::jsonb) into v_security
  from (
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'category',category,'income',income,'expense',expense,'balance',income-expense
  ) order by (income+expense) desc,category),'[]'::jsonb) into v_finance_categories
  from (
    select category,
  ...
  select jsonb_build_object(
    'gates',jsonb_build_object('used',count(*) filter(where is_active and not is_demo),'limit',v_plan.gate_limit),
  ...
  from draborngate.dkd_gate_site_gates where site_id=p_site_id;
