...
DKD_BACKUP_BRANCH="backup/draborngate-v0.3.16-before-v0.3.17-20260802"
DKD_DOWNLOAD_DIR="/sdcard/Download/DraBornGate_Yedekler"
DKD_TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
DKD_BACKUP_ZIP="$DKD_DOWNLOAD_DIR/DraBornGate_Local_Before_Rollback_to_v0.3.16_$DKD_TIMESTAMP.zip"
...
git checkout -B local-v0.3.16-rollback "origin/$DKD_BACKUP_BRANCH"
git clean -fd -e .env -e .env.local -e node_modules
npm install --no-audit --no-fund --package-lock=false
test "$(node -p 'require("./package.json").version')" = "0.3.16"
test "$(node -p 'require("./app.json").expo.android.versionCode')" = "6"
printf '\nLokal proje v0.3.16 sürümüne döndü. GitHub main değişmedi.\nYedek ZIP: %s\nLokal SHA: %s\n' "$DKD_BACKUP_ZIP" "$(git rev-parse HEAD)"
