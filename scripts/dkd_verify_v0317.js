...
function assert(condition, message) { if (!condition) throw new Error(`DKD v0.3.17 doğrulama hatası: ${message}`); }
...
assert(pkg.version === '0.3.17', 'package.json sürümü 0.3.17 değil');
assert(app.version === '0.3.17', 'app.json sürümü 0.3.17 değil');
assert(app.android.versionCode === 7, 'Android versionCode 7 değil');
assert(app.android.package === 'com.draborneagle.draborngate', 'Android paket adı değişmiş');
assert(app.extra.appVersion === '0.3.17' && app.extra.demoDataVersion === '0.3.17' && app.extra.androidVersionCode === 7, 'Expo extra sürüm alanları eşleşmiyor');
assert(version.includes("APP_VERSION = '0.3.17'") && version.includes('ANDROID_VERSION_CODE = 7'), 'merkezi sürüm alanları eşleşmiyor');
assert(pkg.dependencies['expo-iap'] === '4.7.0', 'expo-iap 4.7.0 sabitlenmemiş');

assert(billing.includes('DKD_V0317_PLAY_BILLING_RESTORE'), 'v0.3.17 billing bileşeni yok');
assert(billing.includes('getAvailablePurchases'), 'uygulama açılışında abonelik geri yükleme yok');
...
assert(billing.includes('isOfferPersonalized: false'), 'kişiselleştirilmiş fiyat bildirimi ayarı açıkça belirtilmemiş');
...
console.log('DraBornGate v0.3.17 doğrulaması geçti: versionCode 7, abonelik geri yükleme, sunucu doğrulama, sakin arama ve iki workflow.');
