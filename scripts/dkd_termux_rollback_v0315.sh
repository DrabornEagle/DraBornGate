...
DKD_BACKUP_BRANCH="backup/draborngate-v0.3.15-before-v0.3.16-20260728"
DKD_LOCAL_ROLLBACK_BRANCH="rollback/draborngate-v0.3.15"
DKD_TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
DKD_BACKUP_ZIP="$DKD_DOWNLOAD_DIR/DraBornGate_Local_Before_v0.3.15_Rollback_$DKD_TIMESTAMP.zip"
...
echo "[1/5] Mevcut v0.3.16 yerel kaynak yedekleniyor..."
zip -qr "$DKD_BACKUP_ZIP" . \
  -x 'node_modules/*' 'android/*' '.expo/*' 'dist/*' '.git/*' '*.log'
printf 'Yedek: %s\n' "$DKD_BACKUP_ZIP"

echo "[2/5] v0.3.15 güvenlik dalı alınıyor..."
...
echo "[3/5] Yalnızca yerel proje v0.3.15 geri alma dalına geçiriliyor..."
git checkout -B "$DKD_LOCAL_ROLLBACK_BRANCH" "origin/$DKD_BACKUP_BRANCH"
git clean -fd -e .env -e .env.local -e node_modules

echo "[4/5] v0.3.15 bağımlılık ve kaynak kontrolleri çalıştırılıyor..."
