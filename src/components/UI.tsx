...
export function Panel({ children, style, gradient = false }: PropsWithChildren<{ style?: StyleProp<ViewStyle>; gradient?: boolean }>) {
  ...
  if (gradient) return <AnimatedGradient colors={gradients.panelColorful} style={[styles.panel, style, animated]}><View style={styles.accent} />{children}</AnimatedGradient>;
...
const statusMeta: Record<PassStatus, { label: string; color: string; icon: keyof typeof Ionicons.glyphMap }> = {
  waiting: { label: 'Kod hazır', color: colors.orange, icon: 'keypad' },
...
export function MetricCard({ label, value, icon, tone }: { label: string; value: string; icon: keyof typeof Ionicons.glyphMap; tone: string }) {
  return <Panel style={[styles.metric, { borderColor: `${tone}45` }]} gradient><FloatingView distance={4} duration={1500} style={[styles.metricIcon, { backgroundColor: `${tone}25` }]}><Ionicons name={icon} size={23} color={tone} /></FloatingView><Text style={styles.metricValue}>{value}</Text><Text style={styles.metricLabel}>{label}</Text></Panel>;
}

export function EmptyState({ icon, title, description }: { icon: keyof typeof Ionicons.glyphMap; title: string; description: string }) {
  return <Panel style={styles.empty} gradient><FloatingView style={styles.emptyIcon}><Ionicons name={icon} size={32} color={colors.cyan} /></FloatingView><Text style={styles.emptyTitle}>{title}</Text><Text style={styles.emptyText}>{description}</Text></Panel>;
}
