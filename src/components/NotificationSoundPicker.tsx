...
export function NotificationSoundPicker() {
  const [selected, setSelected] = useState<GateNotificationSoundKey>('bell');
  ...
  return <View style={s.list}>
    <Panel style={s.info} gradient>
      ...
      <View style={s.copy}>
        ...
        <Text style={s.infoText}>Seçim bu cihazda saklanır. Android bildirim ayarları sesi ayrıca sessize alabilir.</Text>
      </View>
    </Panel>
    {GATE_NOTIFICATION_SOUND_OPTIONS.map((item) => {
      const active = selected === item.key;
      return <AnimatedPressable key={item.key} onPress={() => void choose(item.key)}>
        <Panel style={[s.option, active && s.active]} gradient>
          <View style={[s.icon, { backgroundColor: active ? 'rgba(55,216,255,.16)' : 'rgba(139,107,255,.10)' }]}>
            <Ionicons name={item.key === 'silent' ? 'volume-mute' : item.key === 'alert' ? 'warning' : 'musical-notes'} size={23} color={active ? colors.cyan : colors.purple} />
          </View>
          <View style={s.copy}>
...
const s = StyleSheet.create({
  ...
  option: { minHeight: 76, flexDirection: 'row', alignItems: 'center', gap: 9, padding: 10 },
