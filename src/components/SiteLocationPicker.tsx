...
async function searchOpenStreetMap(text: string): Promise<GateMapPoint | undefined> {
  const endpoint = `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&countrycodes=tr&accept-language=tr&q=${encodeURIComponent(text)}`;
  const response = await fetch(endpoint, {
    headers: {
      Accept: 'application/json',
      'Accept-Language': 'tr-TR,tr;q=0.9',
      'User-Agent': 'DraBornGate/0.3.9 (support@draborneagle.com)',
    },
  });
...
export function SiteLocationPicker({ value, address, city, onChange }: { value?: GateMapPoint; address?: string; city?: string; onChange: (point: GateMapPoint) => void }) {
  ...
  useEffect(() => {
    setQuery((current) => current.trim() ? current : [address, city].filter(Boolean).join(', '));
  }, [address, city]);
  ...
  const onMapMessage = (event: WebViewMessageEvent) => {
    try {
      ...
      if (message.type === 'ready') {
        setMapReady(true);
        setMapError('');
      } else if (message.type === 'point' && Number.isFinite(message.latitude) && Number.isFinite(message.longitude)) {
        onChange({ latitude: Number(message.latitude), longitude: Number(message.longitude) });
      } else if (message.type === 'error') {
        setMapError(message.message || 'Harita yüklenemedi.');
      } else if (message.type === 'tileError' && !mapReady) {
        setMapError(message.message || 'Harita döşemeleri yüklenemedi.');
      }
    } catch {
      // Yalnızca harita içeriğinden gelen JSON mesajları işlenir.
    }
  };
  ...
  const search = async () => {
    ...
    setSearching(true);
    try {
      const webPoint = await searchOpenStreetMap(text).catch(() => undefined);
      if (webPoint) {
        setPoint(webPoint);
        return;
      }
      ...
      if (permission.granted) {
        const nativeResults = await Location.geocodeAsync(text);
      ...
      throw new Error('Adres bulunamadı. İlçe, mahalle, cadde ve şehir yazarak tekrar dene veya haritada noktaya dokun.');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.labelRow}>
        ...
        <AnimatedPressable onPress={() => setExpanded(true)}>
          <View style={styles.expandLabelButton}>
            <Ionicons name="expand" size={17} color={colors.cyan} />
            <Text style={styles.expandLabelText}>HARİTAYI BÜYÜT</Text>
          </View>
        </AnimatedPressable>
      </View>
      ...
      <View style={styles.searchRow}>
        ...
        <AnimatedPressable onPress={() => void search()} disabled={searching}>
          <View style={styles.searchButton}>{searching ? <ActivityIndicator color={colors.white} /> : <Ionicons name="navigate" size={21} color={colors.white} />}</View>
        </AnimatedPressable>
      </View>
      ...
      <View style={styles.mapFrame}>
        <WebView
          ...
          style={styles.map}
          {...mapProps}
          onLoadStart={() => { setMapReady(false); setMapError(''); }}
          onError={() => setMapError('Harita görünümü açılamadı. İnternet bağlantısını kontrol edin.')}
        />
        {!mapReady && !mapError ? <View pointerEvents="none" style={styles.loading}><ActivityIndicator size="large" color={colors.cyan} /><Text style={styles.loadingText}>Harita yükleniyor</Text></View> : null}
        ...
        <AnimatedPressable onPress={() => setExpanded(true)}><View style={styles.expand}><Ionicons name="expand" size={24} color={colors.white} /></View></AnimatedPressable>
        <View pointerEvents="none" style={styles.mapHint}><Ionicons name="hand-left" size={16} color={colors.white} /><Text style={styles.mapHintText}>Haritaya dokun veya pini sürükle</Text></View>
      </View>

      <Text style={styles.attribution}>Harita: OpenStreetMap katkıcıları • Konum yalnızca site ayarı için kullanılır.</Text>
      <View style={styles.footer}>
        <AnimatedPressable onPress={() => void useMyLocation()} disabled={searching}><View style={styles.locationButton}><Ionicons name="locate" size={19} color={colors.green} /><Text style={styles.locationText}>KONUMUMU KULLAN</Text></View></AnimatedPressable>
        <View style={styles.coordinateBox}><Text style={styles.coordinateLabel}>{value ? 'KONUM HAZIR' : 'KONUM BEKLENİYOR'}</Text><Text style={styles.coordinateText}>{value ? `${value.latitude.toFixed(5)}, ${value.longitude.toFixed(5)}` : 'Haritadan konum seç'}</Text></View>
      </View>

      <Modal visible={expanded} animationType="slide" onRequestClose={() => setExpanded(false)}>
        <SafeAreaView style={styles.fullSafe} edges={['top', 'bottom', 'left', 'right']}>
          <View style={styles.fullHeader}>
            <View style={styles.fullTitleWrap}><Text style={styles.fullKicker}>SİTE KONUMU</Text><Text style={styles.fullTitle}>Büyük Harita</Text><Text style={styles.fullSub}>Yakınlaştır, uzaklaştır, haritayı sürükle ve doğru noktaya dokun.</Text></View>
            <AnimatedPressable onPress={() => setExpanded(false)}><View style={styles.close}><Ionicons name="close" size={25} color={colors.white} /></View></AnimatedPressable>
          </View>
          <View style={styles.fullMap}><WebView key={`full-${mapKey}-${expanded ? 1 : 0}`} ref={fullRef} style={styles.map} {...mapProps} /></View>
          <View style={styles.fullBottom}>
            <View style={styles.fullCoordinate}><Text style={styles.coordinateLabel}>{value ? 'SEÇİLEN KONUM' : 'KONUM BEKLENİYOR'}</Text><Text style={styles.fullCoordinateText}>{value ? `${value.latitude.toFixed(6)}, ${value.longitude.toFixed(6)}` : 'Haritadan bir nokta seç'}</Text></View>
            <AnimatedPressable onPress={() => setExpanded(false)}><View style={styles.useButton}><Ionicons name="checkmark-done" size={21} color={colors.background} /><Text style={styles.useButtonText}>BU KONUMU KULLAN</Text></View></AnimatedPressable>
          </View>
        </SafeAreaView>
      </Modal>
    </View>
...
const styles = StyleSheet.create({
  ...
  label: { color: colors.textSoft, fontSize: 12, fontWeight: '900', letterSpacing: 0.6 },
  expandLabelButton: { minHeight: 34, borderRadius: 12, paddingHorizontal: 10, borderWidth: 1, borderColor: 'rgba(55,216,255,.40)', backgroundColor: 'rgba(55,216,255,.09)', flexDirection: 'row', alignItems: 'center', gap: 6 },
  expandLabelText: { color: colors.cyan, fontSize: 8.5, fontWeight: '900' },
  ...
  mapHelp: { position: 'absolute', left: 16, right: 16, top: 36, padding: 15, borderRadius: 18, backgroundColor: 'rgba(5,15,27,.95)', alignItems: 'center', borderWidth: 1, borderColor: 'rgba(255,179,92,.45)' },
  helpTitle: { color: colors.text, fontWeight: '900', marginTop: 7 },
  helpText: { color: colors.textSoft, fontSize: 10, lineHeight: 15, textAlign: 'center', marginTop: 5 },
  retry: { minHeight: 38, borderRadius: 13, borderWidth: 1, borderColor: 'rgba(55,216,255,.45)', backgroundColor: 'rgba(55,216,255,.08)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingHorizontal: 12, marginTop: 10 },
  retryText: { color: colors.cyan, fontSize: 9, fontWeight: '900' },
  attribution: { color: colors.textMuted, fontSize: 8.5, lineHeight: 13, textAlign: 'center' },
  ...
  fullSafe: { flex: 1, backgroundColor: colors.background },
  fullHeader: { minHeight: 105, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, borderBottomWidth: 1, borderBottomColor: colors.borderStrong },
  fullTitleWrap: { flex: 1 },
  fullKicker: { color: colors.cyan, fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  fullTitle: { color: colors.white, fontSize: 23, fontWeight: '900', marginTop: 3 },
  fullSub: { color: colors.textSoft, fontSize: 11, lineHeight: 16, marginTop: 4 },
  close: { width: 50, height: 50, borderRadius: 17, borderWidth: 1, borderColor: colors.borderStrong, backgroundColor: 'rgba(255,255,255,.06)', alignItems: 'center', justifyContent: 'center' },
  fullMap: { flex: 1, margin: 12, borderRadius: 23, overflow: 'hidden', borderWidth: 1.5, borderColor: colors.borderStrong },
  fullBottom: { padding: 14, gap: 12, borderTopWidth: 1, borderTopColor: colors.borderStrong },
  fullCoordinate: { alignItems: 'center' },
  fullCoordinateText: { color: colors.white, fontSize: 13, fontWeight: '800', marginTop: 4 },
  useButton: { minHeight: 58, borderRadius: 19, backgroundColor: colors.green, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  useButtonText: { color: colors.background, fontSize: 12, fontWeight: '900' },
});
