...
export function CourierHome({ onCreatePass, onOpenPasses, onOpenSettings }: { onCreatePass: () => void; onOpenPasses: () => void; onOpenSettings: () => void }) {
  ...
  const markArrived = async () => {
    ...
    try {
      ...
      Alert.alert('Kapıya geldin', `Güvenliğe varış kaydı gönderildi. Kapıdaki görevliye ${active.approvalCode ?? '6 haneli'} tek kullanımlık kodunu söyle.`);
    } catch (error) {
