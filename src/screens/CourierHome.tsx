...
export function CourierHome({ onCreatePass, onOpenPasses, onOpenSettings }: { onCreatePass: () => void; onOpenPasses: () => void; onOpenSettings: () => void }) {
  ...
  const promptedPass = useRef<string>();
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!permission.granted) {
        setTracking(false);
        Alert.alert('Konum izni gerekli', 'AirPass kapıya olan mesafeyi yalnızca uygulama açıkken kontrol eder.');
        return;
      }
      subscription = await Location.watchPositionAsync({ accuracy: Location.Accuracy.High, distanceInterval: 4, timeInterval: 3000 }, (position) => {
        ...
        if (meters <= 30 && promptedPass.current !== active.id && !active.airpassSentAt) {
          ...
          void showGateNotification('AirPass hazır', `${selectedGate.name} kapısına ${meters} metre kaldı. Güvenliğe göndermek ister misin?`, { passId: active.id, distance: meters });
          Alert.alert('Kapıya 30 metre kaldı', 'AirPass bilgisi güvenliğe gönderilsin mi?', [
  ...
  const sendAirPassNow = async () => {
    ...
    try {
      ...
      Alert.alert('AirPass gönderildi', 'Konum ve kapı mesafesi güvenlik paneline iletildi.');
    } catch (error) {
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void refresh()} tintColor={colors.cyan} />} showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
    <FadeInView style={s.header}><View><Text style={s.eyebrow}>KURYE OPERASYONU</Text><Text style={s.title}>{profile?.fullName.split(' ')[0] || 'Kurye'} 👋</Text><Text style={s.subtitle}>{courierProfile?.platform || 'DraBornGo'} • {courierProfile?.plate || 'Plaka eklenmedi'}</Text></View><LiveBadge label="SUPABASE" /></FadeInView>
    <FadeInView delay={70}><LinearGradient colors={gradients.courier} style={s.hero}><View style={s.heroTop}><FloatingView style={s.motorShell} distance={6}><RacingMotorcycle color={colors.cyan} accentColor={colors.white} size={84} /></FloatingView><View style={s.heroCopy}><Text style={s.heroKicker}>CourierPass + AirPass</Text><Text style={s.heroTitle}>Kapıda bekleme yok.</Text><Text style={s.heroText}>Sipariş görselini okut, geçişini gönder ve kapıya 30 metre kala konumunu doğrula.</Text></View></View><AnimatedPressable onPress={canCreate ? onCreatePass : onOpenSettings}><View style={s.button}><Ionicons name={canCreate ? 'paper-plane' : 'settings'} size={21} color={colors.background} /><Text style={s.buttonText}>{canCreate ? 'YENİ GEÇİŞ TALEBİ' : 'DEMO VEYA SİTE VERİSİ EKLE'}</Text><Ionicons name="arrow-forward" size={20} color={colors.background} /></View></AnimatedPressable></LinearGradient></FadeInView>
