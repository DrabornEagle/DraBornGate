...
export function SecurityHome() {
  ...
  const verifyCourier = async () => {
    if (!matchedPassId) return;
    try {
      await gate.updatePassStatus(matchedPassId, 'completed', undefined, courierCode);
      setMatchedPassId(undefined); setCourierCode('');
      Alert.alert('Giriş verildi', 'Kurye bulundu, kod eşleşti ve tek kullanımlık hak tüketildi.');
  ...
  return <ScrollView refreshControl={<RefreshControl refreshing={gate.refreshing} onRefresh={() => void gate.refresh()} tintColor={colors.green} />} showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
    <FadeInView style={s.header}><View><Text style={s.eyebrow}>GÜVENLİK OPERASYONU</Text><Text style={s.title}>Kapı geçiş merkezi</Text><Text style={s.sub}>Kurye kodu, ziyaretçi ve denetim işlemleri</Text></View><LiveBadge /></FadeInView>
    ...
    {mode === 'courier' ? <>
      ...
      <Panel style={s.lookup} gradient><Text style={s.lookupTitle}>Kuryenin verdiği 6 haneli kodu gir</Text><Text style={s.lookupHelp}>Kod kartta önceden görünür; kapıya gelen kuryeden aynı kodu söylemesini iste.</Text><View style={s.lookupRow}><TextInput value={courierCode} onChangeText={(value) => { setCourierCode(value.replace(/\D/g, '').slice(0, 6)); setMatchedPassId(undefined); }} keyboardType="number-pad" maxLength={6} style={s.codeInput} placeholder="000000" placeholderTextColor={colors.textMuted} /><AnimatedPressable onPress={() => void lookupCourier()} disabled={searching}><View style={s.searchButton}><Ionicons name="search" size={23} color={colors.background} /><Text style={s.searchText}>{searching ? 'ARANIYOR' : 'KURYEYİ BUL'}</Text></View></AnimatedPressable></View></Panel>
      {matchedPass ? <View style={s.match}><PassCard pass={matchedPass} showImage imageFullscreen showApprovalCode={false} revealApprovalCode /><AnimatedPressable onPress={() => void verifyCourier()}><LinearGradient colors={gradients.success} style={s.verify}><Ionicons name="checkmark-done" size={22} color={colors.background} /><Text style={s.verifyText}>KOD EŞLEŞTİ • GİRİŞ VER</Text></LinearGradient></AnimatedPressable></View> : courierCode.length === 6 ? <EmptyState icon="search" title="Kodu ara" description="Kurye “Kapıya Geldim” dedikten sonra arama düğmesine dokun." /> : null}
      ...
      <View style={s.filters}>{(['waiting', 'approved', 'arrived', 'all'] as PassFilter[]).map((item) => <AnimatedPressable key={item} containerStyle={s.filterWrap} onPress={() => setFilter(item)}><View style={[s.filter, filter === item && s.active]}><Text style={[s.filterText, filter === item && s.activeText]}>{item === 'waiting' ? 'Kod Hazır' : item === 'approved' ? 'İncelendi' : item === 'arrived' ? 'Kapıda' : 'Aktif'}</Text></View></AnimatedPressable>)}</View>
      <View style={s.list}>{courierQueue.length ? courierQueue.map((pass) => <View key={pass.id} style={s.passWrap}><PassCard pass={pass} showImage imageFullscreen showApprovalCode={false} revealApprovalCode />
        {pass.airpassSentAt ? <Panel style={s.airInfo} gradient><Ionicons name="navigate" size={25} color={pass.locationVerified ? colors.green : colors.cyan} /><View style={s.copy}><Text style={s.airTitle}>Akıllı Geçiş güvenliğe gönderildi</Text><Text style={s.airText}>{pass.lastDistanceM != null ? `${Math.round(pass.lastDistanceM)} metre` : 'Mesafe yok'} • {pass.locationVerified ? 'Konum doğrulandı' : '30 metre dışında'}</Text></View></Panel> : null}
        {['waiting', 'approved', 'arrived'].includes(pass.status) ? rejecting === pass.id ? <Panel style={s.rejectPanel}><TextInput value={reason} onChangeText={setReason} placeholder="Reddetme sebebi" placeholderTextColor={colors.textMuted} style={s.input} /><View style={s.actions}><Small title="Vazgeç" tone={colors.textSoft} onPress={() => { setRejecting(undefined); setReason(''); }} /><Small title="Sebebiyle Reddet" tone={colors.red} onPress={() => void reject(pass)} /></View></Panel> : <View style={s.actions}><Small title="Reddet" tone={colors.red} onPress={() => setRejecting(pass.id)} />{pass.status === 'arrived' ? <View style={s.waitCode}><Ionicons name="keypad" size={21} color={colors.purple} /><Text style={s.waitCodeText}>Karttaki kodu kuryeden iste • Üstte eşleştir</Text></View> : <View style={s.waitCode}><Ionicons name="time" size={21} color={colors.orange} /><Text style={s.waitCodeText}>Kod hazır • Kuryenin kapıya gelmesi bekleniyor</Text></View>}</View> : null}
...
function Choice({ active, label, sub, onPress }: { active: boolean; label: string; sub?: string; onPress: () => void }) { return <AnimatedPressable onPress={onPress}><View style={[s.choice, active && s.active]}><Ionicons name="enter" size={19} color={active ? colors.green : colors.textMuted} /><View><Text style={s.choiceTitle}>{label}</Text>{sub ? <Text style={s.choiceSub}>{sub}</Text> : null}</View></View></AnimatedPressable>; }
...
const s = StyleSheet.create({
  content: { padding: spacing.md, paddingTop: 12, paddingBottom: 114, gap: 19 }, header: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingRight: 58 }, eyebrow: { color: colors.green, fontSize: 12, fontWeight: '900', letterSpacing: 1 }, title: { color: colors.text, fontSize: 29, fontWeight: '900', marginTop: 4 }, sub: { color: colors.textSoft, fontSize: 14, marginTop: 4 }, copy: { flex: 1 },
  hero: { borderRadius: radius.xl, padding: 20, flexDirection: 'row', alignItems: 'center', gap: 14, borderWidth: 1, borderColor: 'rgba(255,255,255,.22)' }, heroIcon: { width: 64, height: 64, borderRadius: 21, backgroundColor: 'rgba(255,255,255,.16)', alignItems: 'center', justifyContent: 'center' }, heroLabel: { color: 'rgba(255,255,255,.78)', fontSize: 12, fontWeight: '900' }, heroValue: { color: colors.white, fontSize: 31, fontWeight: '900' }, heroText: { color: 'rgba(255,255,255,.84)', fontSize: 14, lineHeight: 20, marginTop: 3 }, metrics: { flexDirection: 'row', gap: 8 },
  modeRow: { flexDirection: 'row', gap: 9 }, modeWrap: { flex: 1 }, mode: { height: 56, borderRadius: 17, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, modeActive: { borderColor: colors.green, backgroundColor: 'rgba(67,231,162,.10)' }, modeText: { color: colors.textMuted, fontSize: 14, fontWeight: '900' }, modeTextActive: { color: colors.green },
  lookup: { gap: 9, borderColor: 'rgba(139,107,255,.45)' }, lookupTitle: { color: colors.text, fontSize: 17, fontWeight: '900', textAlign: 'center' }, lookupHelp: { color: colors.textSoft, fontSize: 13, lineHeight: 19, textAlign: 'center' }, lookupRow: { flexDirection: 'row', gap: 8 }, codeInput: { flex: 1, height: 64, borderRadius: 17, borderWidth: 1, borderColor: colors.purple, color: colors.text, fontSize: 28, fontWeight: '900', letterSpacing: 7, textAlign: 'center' }, searchButton: { width: 116, height: 64, borderRadius: 17, backgroundColor: colors.green, alignItems: 'center', justifyContent: 'center', gap: 2 }, searchText: { color: colors.background, fontSize: 10, fontWeight: '900' }, match: { gap: 9 }, verify: { height: 58, borderRadius: 18, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 }, verifyText: { color: colors.background, fontSize: 13, fontWeight: '900' },
  horizontal: { gap: 8, paddingRight: 10 }, choice: { minWidth: 150, height: 56, borderRadius: 16, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 12 }, choiceTitle: { color: colors.text, fontSize: 14, fontWeight: '900' }, choiceSub: { color: colors.textSoft, fontSize: 11, marginTop: 2 },
  filters: { flexDirection: 'row', gap: 7 }, filterWrap: { flex: 1 }, filter: { height: 47, borderRadius: 15, borderWidth: 1, borderColor: colors.border, alignItems: 'center', justifyContent: 'center' }, active: { borderColor: colors.green, backgroundColor: 'rgba(67,231,162,.09)' }, filterText: { color: colors.textMuted, fontSize: 12, fontWeight: '900' }, activeText: { color: colors.green }, list: { gap: 16 }, passWrap: { gap: 10 },
  airInfo: { flexDirection: 'row', alignItems: 'center', gap: 10, borderColor: 'rgba(55,216,255,.36)' }, airTitle: { color: colors.cyan, fontSize: 15, fontWeight: '900' }, airText: { color: colors.textSoft, fontSize: 13, marginTop: 3 }, actions: { flexDirection: 'row', gap: 8 }, actionWrap: { flex: 1 }, small: { minHeight: 54, borderRadius: 17, borderWidth: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 8 }, smallText: { fontSize: 12, fontWeight: '900', textAlign: 'center' }, rejectPanel: { gap: 9, borderColor: 'rgba(255,101,125,.36)' }, input: { height: 52, borderRadius: 15, borderWidth: 1, borderColor: colors.border, color: colors.text, paddingHorizontal: 12, fontSize: 15 }, waitCode: { flex: 1, minHeight: 54, borderRadius: 17, borderWidth: 1, borderColor: 'rgba(139,107,255,.45)', backgroundColor: 'rgba(139,107,255,.10)', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 9 }, waitCodeText: { flex: 1, color: colors.purple, fontSize: 12, lineHeight: 17, fontWeight: '900', textAlign: 'center' },
  visitorLookup: { gap: 9 }, visitorCode: { height: 68, borderRadius: 18, borderWidth: 1, borderColor: colors.green, color: colors.text, fontSize: 31, fontWeight: '900', letterSpacing: 8, textAlign: 'center' }, visitorCard: { gap: 12, borderColor: 'rgba(67,231,162,.38)' }, visitorTop: { flexDirection: 'row', alignItems: 'center', gap: 10 }, visitorIcon: { width: 54, height: 54, borderRadius: 18, backgroundColor: 'rgba(67,231,162,.13)', alignItems: 'center', justifyContent: 'center' }, visitorName: { color: colors.text, fontSize: 19, fontWeight: '900' }, visitorMeta: { color: colors.textSoft, fontSize: 13, marginTop: 4 },
});
