...
import { Alert, Animated, Modal, RefreshControl, ScrollView, StyleSheet, Text, TextInput, View } from 'react-native';
...
export function SecurityHome() {
  ...
  const verifyMotion = useRef(new Animated.Value(0)).current;
  useEffect(() => { const loop = Animated.loop(Animated.sequence([Animated.timing(verifyMotion, { toValue: 1, duration: 850, useNativeDriver: true }), Animated.timing(verifyMotion, { toValue: 0, duration: 850, useNativeDriver: true })])); loop.start(); return () => loop.stop(); }, [verifyMotion]);
  ...
  return <>
    <ScrollView refreshControl={<RefreshControl refreshing={gate.refreshing} onRefresh={() => void gate.refresh()} tintColor={colors.green} />} showsVerticalScrollIndicator={false} contentContainerStyle={s.content}>
      ...
      {mode === 'courier' ? <>
        ...
        {matchedPass ? <View style={s.match}><PassCard pass={matchedPass} showImage imageFullscreen securityCodeMode /><Animated.View style={{ transform: [{ scale: verifyMotion.interpolate({ inputRange: [0, 1], outputRange: [1, 1.035] }) }] }}><AnimatedPressable onPress={() => void verifyCourier()}><LinearGradient colors={[colors.green, colors.cyan, colors.purple]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={s.verify}><Animated.View pointerEvents="none" style={[s.verifySweep, { opacity: verifyMotion.interpolate({ inputRange: [0, 1], outputRange: [.08, .42] }), transform: [{ translateX: verifyMotion.interpolate({ inputRange: [0, 1], outputRange: [-110, 260] }) }, { rotate: '-18deg' }] }]} /><Ionicons name="checkmark-done" size={23} color={colors.background} /><Text style={s.verifyText}>KOD EŞLEŞTİ • GİRİŞ VER</Text><Ionicons name="enter" size={22} color={colors.background} /></LinearGradient></AnimatedPressable></Animated.View></View> : courierCode.length === 6 ? <EmptyState icon="search" title="Kodu ara" description="Kurye “Kapıya Geldim” dedikten sonra arama düğmesine dokun." /> : null}
        ...
        <View style={s.list}>{courierQueue.length ? courierQueue.map((pass) => <View key={pass.id} style={s.passWrap}><PassCard pass={pass} showImage imageFullscreen securityCodeMode onMatchCode={() => openCardMatcher(pass)} />
          ...
          {['waiting', 'approved', 'arrived'].includes(pass.status) ? rejecting === pass.id ? <Panel style={s.rejectPanel}><TextInput value={reason} onChangeText={setReason} placeholder="Reddetme sebebi" placeholderTextColor={colors.textMuted} style={s.input} /><View style={s.actions}><Small title="Vazgeç" tone={colors.textSoft} onPress={() => { setRejecting(undefined); setReason(''); }} /><Small title="Sebebiyle Reddet" tone={colors.red} onPress={() => void reject(pass)} /></View></Panel> : <View style={s.actions}><Small title="Reddet" tone={colors.red} onPress={() => setRejecting(pass.id)} /></View> : null}
...
const s = StyleSheet.create({
  ...
  lookup: { gap: 10, borderColor: 'rgba(139,107,255,.45)' }, lookupTitle: { color: colors.text, fontSize: 14, fontWeight: '900', textAlign: 'center' }, lookupRow: { flexDirection: 'row', gap: 8 }, codeInput: { flex: 1, height: 60, borderRadius: 17, borderWidth: 1, borderColor: colors.purple, color: colors.text, fontSize: 25, fontWeight: '900', letterSpacing: 6, textAlign: 'center' }, searchButton: { width: 110, height: 60, borderRadius: 17, backgroundColor: colors.green, alignItems: 'center', justifyContent: 'center', gap: 2 }, searchText: { color: colors.background, fontSize: 9, fontWeight: '900' }, match: { gap: 9 }, verify: { height: 62, borderRadius: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 9, overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255,255,255,.45)' }, verifySweep: { position: 'absolute', top: -35, bottom: -35, width: 42, backgroundColor: 'rgba(255,255,255,.70)' }, verifyText: { color: colors.background, fontSize: 13, fontWeight: '900', letterSpacing: .25 },
