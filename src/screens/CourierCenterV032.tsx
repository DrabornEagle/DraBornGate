...
export function CourierCenterV032() {
  ...
  return <View style={s.flex}>
    <View style={s.tabs}>
      ...
      <NavCard active={tab === 'packages'} icon="diamond" title="Kurye Paketleri" text={hasCourierPackage ? 'Paketin aktif • detayları aç' : 'Renkli paketleri keşfet'} tone={colors.magenta} colorful attention={!hasCourierPackage} motion={!hasCourierPackage} onPress={() => setTab('packages')} />
    </View>
  ...
  return <Animated.View style={[s.navWrap, attention && { transform: [{ scale: dkd_motion.interpolate({ inputRange: [0, 1], outputRange: [1, 1.035] }) }] }]}>
    <AnimatedPressable containerStyle={s.navPress} onPress={onPress}>
      <LinearGradient colors={dkd_gradient as [string, string, ...string[]]} style={[s.nav, active && { borderColor: tone }, colorful && s.navColorful]}>
        ...
        <View style={s.copy}><Text style={[s.navTitle, (active || colorful) && { color: tone }]}>{title}</Text><Text style={[s.navText, colorful && s.navTextColorful]}>{text}</Text>{attention ? <Text style={s.navCallout}>DOKUN • PAKETİNİ SEÇ</Text> : null}</View>
