...
export function CourierCenterV032() {
  ...
  const hasCourierPackage = Boolean(center?.subscription && ['active', 'trialing'].includes(center.subscription.status) && center.effective_plan?.code !== 'courier_starter');
  return <View style={s.flex}>
    <View style={s.tabs}>
      ...
      <NavCard active={tab === 'packages'} icon="diamond" title="Kurye Paketleri" text={hasCourierPackage ? 'Aktif paketini ve kullanımını yönet' : 'Paketleri karşılaştır • sana uygun planı seç'} tone={colors.magenta} colorful attention={!hasCourierPackage} motion={!hasCourierPackage} onPress={() => setTab('packages')} />
    </View>
...
function NavCard({ active, icon, title, text, tone, colorful = false, attention = false, motion = true, onPress }: { active: boolean; icon: keyof typeof Ionicons.glyphMap; title: string; text: string; tone: string; colorful?: boolean; attention?: boolean; motion?: boolean; onPress: () => void }) {
  const dkd_motion = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    if (!motion) { dkd_motion.stopAnimation(); dkd_motion.setValue(0); return; }
    const dkd_loop = Animated.loop(Animated.sequence([
      Animated.timing(dkd_motion, { toValue: 1, duration: 1050, useNativeDriver: true }),
      Animated.timing(dkd_motion, { toValue: 0, duration: 1050, useNativeDriver: true }),
    ]));
    dkd_loop.start();
    return () => dkd_loop.stop();
  }, [dkd_motion, motion]);
  const dkd_gradient = colorful
    ? (active ? ['rgba(222,85,255,.42)', 'rgba(55,216,255,.25)', 'rgba(255,179,92,.17)'] : ['rgba(124,68,185,.30)', 'rgba(18,86,118,.28)', 'rgba(63,48,102,.32)'])
    : (active ? [`${tone}30`, 'rgba(13,32,51,.98)'] : ['rgba(15,38,59,.95)', 'rgba(8,24,40,.98)']);
  const dkd_icon = <View style={[s.navIcon, { backgroundColor: `${tone}1F` }]}><Ionicons name={icon} size={25} color={active || colorful ? tone : colors.textMuted} /></View>;
  return <Animated.View style={[s.navWrap, attention && { transform: [{ scale: dkd_motion.interpolate({ inputRange: [0, 1], outputRange: [1, 1.035] }) }] }]}>
    <AnimatedPressable containerStyle={s.navPress} onPress={onPress}>
      <LinearGradient colors={dkd_gradient as [string, string, ...string[]]} style={[s.nav, active && { borderColor: tone }, colorful && s.navColorful]}>
        {attention ? <Animated.View pointerEvents="none" style={[s.navAttention, { opacity: dkd_motion.interpolate({ inputRange: [0, 1], outputRange: [.18, .72] }) }]} /> : null}
        {motion ? <FloatingView distance={attention ? 5 : 3} duration={attention ? 1150 : 1500}>{dkd_icon}</FloatingView> : dkd_icon}
        <View style={s.copy}><Text style={[s.navTitle, (active || colorful) && { color: tone }]}>{title}</Text><Text style={[s.navText, colorful && s.navTextColorful]}>{text}</Text>{attention ? <Text style={s.navCallout}>DOKUN • PLANLARI GÖR</Text> : null}</View>
        {active ? <PulseDot color={tone} size={7} /> : attention ? <Animated.View style={{ opacity: dkd_motion.interpolate({ inputRange: [0, 1], outputRange: [.45, 1] }) }}><Ionicons name="sparkles" size={20} color={colors.orange} /></Animated.View> : <Ionicons name="chevron-forward" size={19} color={colors.textMuted} />}
      </LinearGradient>
    </AnimatedPressable>
  </Animated.View>;
}
